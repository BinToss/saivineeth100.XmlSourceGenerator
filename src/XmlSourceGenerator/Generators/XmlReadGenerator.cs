...
namespace XmlSourceGenerator.Generators
{
    ...
    public partial class XmlReadGenerator
    {
        ...
        public void GenerateReadMethod(INamedTypeSymbol classSymbol)
        {
            ...
            // recurse classes for IXmlStreamable implementation
            bool isNew = false;
            INamedTypeSymbol? symbol = classSymbol;
            while (symbol != null)
            {
                symbol = symbol.BaseType;
                isNew = symbol != null && (
                    ImplementsIXmlStreamable(symbol) ||
                    symbol.GetAttributes().Any(a => a.AttributeClass?.Name == "XmlAutoGeneratedAttribute")
                );
                if (isNew)
                    break;
            }
